...
glm::highp_mat4 Util::GetRotationMatrix(glm::vec3 eulerAngles) { return glm::eulerAngleXYZ(-(eulerAngles.x), -(eulerAngles.y), -(eulerAngles.z)); }

glm::vec3 Util::NormalizeVector(glm::vec3 p) { return glm::normalize(p); }
...
glm::vec3 Util::RotMatrixToEuler(RE::NiMatrix3 matrixIn)
{
	...
	auto rotMat = glm::mat4({ ent[0][0], ent[1][0], ent[2][0], ent[0][1], ent[1][1], ent[2][1], ent[0][2], ent[1][2], ent[2][2] });
...
bool Util::IsRoughlyEqual(float first, float second, float maxDif) { return abs(first - second) <= maxDif; }
...
glm::quat Util::GetInverseRotation(glm::quat rotIn) { return glm::inverse(rotIn); }

glm::vec3 Util::EulerRotationToVector(glm::vec3 rotIn) { return glm::vec3(cos(rotIn.y) * cos(rotIn.x), sin(rotIn.y) * cos(rotIn.x), sin(rotIn.x)); }
